...
import { Timer, Play, Square, Coffee, Flame, Calendar, Clock, Edit2, Check, X, AlertTriangle } from 'lucide-react';
...
export default function FastingTracker() {
  ...
  const [customStartTimeInput, setCustomStartTimeInput] = useState<number>(Date.now());
  ...
  const handlePartChange = (dateVal: string, hourVal: number, minVal: number) => {
  ...
  const activeStart = isFasting ? (startTime || Date.now()) : customStartTimeInput;
  const targetTime = activeStart + fastDurationMs;
  const msLeft = isFasting 
    ? (targetTime ? Math.max(0, targetTime - now) : fastDurationMs)
    : fastDurationMs;
  ...
  const progressPct = isFasting && startTime
  ...
  return (
    <div className="flex flex-col gap-4 animate-in fade-in duration-300">
      ...
      <div className="bg-white dark:bg-slate-800 border border-[#E2E8F0] dark:border-slate-700 rounded-[24px] p-4 shadow-xs" id="fasting-plan-selector">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
          {FASTING_PLANS.map(plan => (
            ...
            >
              {plan.id} <span className="font-medium text-[10px] ml-1 opacity-80">{plan.name}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 border border-[#E2E8F0] dark:border-slate-700 rounded-[24px] p-5 shadow-xs" id="fasting-time-adjuster">
        <h3 className="text-xs font-extrabold text-[#64748B] dark:text-slate-400 uppercase tracking-wider mb-2.5 flex items-center justify-between">
          <span className="flex items-center gap-1"><Clock className="w-4 h-4 text-[#3B82F6]" /> 단식 목표 시간대 조율</span>
          {isFasting && (
            <button 
              type="button" 
          ...
          <div className="flex flex-col gap-3 animate-in fade-in duration-200">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="flex flex-col gap-1">
                <label className="text-[11px] font-black text-slate-500 dark:text-slate-400 flex items-center gap-1">
                  <Calendar className="w-3.5 h-3.5 text-[#3B82F6]" /> 단식 시작 일시 선택 (30분 단위)
                </label>
                <div className="flex gap-2">
                  {/* Date Input */}
                  <input 
                    type="date"
                    value={getFastingParts(isFasting ? (startTime || Date.now()) : customStartTimeInput).dateStr}
                    onChange={(e) => {
                      const parts = getFastingParts(isFasting ? (startTime || Date.now()) : customStartTimeInput);
                  ...
                  {/* Hour Select */}
                  <select
                    value={getFastingParts(isFasting ? (startTime || Date.now()) : customStartTimeInput).hour}
                    onChange={(e) => {
                      const parts = getFastingParts(isFasting ? (startTime || Date.now()) : customStartTimeInput);
                      handlePartChange(parts.dateStr, Number(e.target.value), parts.minute);
                    }}
                  ...
                  {/* Minute Select */}
                  <select
                    value={getFastingParts(isFasting ? (startTime || Date.now()) : customStartTimeInput).minute}
                    onChange={(e) => {
                      const parts = getFastingParts(isFasting ? (startTime || Date.now()) : customStartTimeInput);
                      handlePartChange(parts.dateStr, parts.hour, Number(e.target.value));
                    }}
                    className="w-20 h-11 border border-[#E2E8F0] dark:border-slate-700 rounded-xl px-2 bg-[#F8FAFC] dark:bg-slate-900 text-xs font-black focus:outline-none focus:border-[#3B82F6] dark:text-white text-slate-800"
              ...
              <div className="flex flex-col justify-center p-3 border border-dashed border-indigo-200 dark:border-indigo-900/50 rounded-xl bg-indigo-50/30 dark:bg-indigo-950/20">
                <p className="text-[10px] font-extrabold text-indigo-500 dark:text-indigo-400">예정 종료 (목표 {selectedPlan.fast}시간)</p>
            ...
            {!isFasting && (
              <div className="flex items-center gap-1.5 text-[10px] text-neutral-500 dark:text-neutral-400 bg-neutral-50 dark:bg-neutral-800/20 p-2.5 rounded-lg border border-neutral-100 dark:border-slate-800">
                ...
                <div>어제 밤이나 과거 시점부터 단식을 시작했다면, 위 시계를 돌려 시작 시간을 소급 설정할 수 있습니다.</div>
              </div>
            )}
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-3.5 mt-1.5 animate-in fade-in duration-200">
            <div className="p-3.5 bg-[#FFF1F2] dark:bg-rose-950/20 border border-rose-100 dark:border-rose-950 rounded-[18px]">
              <p className="text-[10px] font-bold text-rose-500 dark:text-rose-400 flex items-center gap-1 block">
                <span className="w-1.5 h-1.5 rounded-full bg-rose-500" /> 단식 시작 시점
              </p>
            ...
            <div className="p-3.5 bg-[#F0FDF4] dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-950 rounded-[18px]">
              <p className="text-[10px] font-bold text-[#16A34A] flex items-center gap-1 block">
                <span className="w-1.5 h-1.5 rounded-full bg-[#16A34A]" /> 단식 목표 시점
              </p>
      ...
      <div className="bg-white dark:bg-slate-800 border border-[#E2E8F0] dark:border-slate-700 rounded-[24px] p-6 shadow-xs flex flex-col items-center relative overflow-hidden" id="fasting-visual-timer">
        
        {/* Active phase state tag */}
        <div className={`mb-6 px-4 py-1.5 rounded-full text-xs font-black shadow-inner flex items-center gap-1.5 transition-colors duration-500 ${
          isFasting 
            ? isCompleted ? 'bg-emerald-100 dark:bg-emerald-950/50 text-[#16A34A] dark:text-emerald-400' : 'bg-indigo-100 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400'
            : 'bg-orange-100 dark:bg-orange-950/50 text-orange-600 dark:text-orange-400'
        }`}>
          {isFasting ? (
            isCompleted ? <><Flame className="w-4 h-4 text-emerald-500 animate-bounce"/> 목표 달성 완료!</> : <><Timer className="w-4 h-4 animate-pulse"/> 단식 진행 중 (시간 흘러가는 중)</>
          ) : (
        ...
        <div className="relative flex items-center justify-center w-64 h-64">
          <svg className="absolute w-full h-full transform -rotate-90" viewBox="0 0 256 256" width="100%" height="100%">
            {/* Background ring */}
            <circle
              cx="128" cy="128" r={radius}
              stroke="var(--circle-bg, #F1F5F9)" strokeWidth="18" fill="transparent"
            ...
            <circle
              cx="128" cy="128" r={radius}
              stroke={isFasting && isCompleted ? '#10B981' : isFasting ? '#3D82F6' : '#EFF6FF'}
              strokeWidth="18"
              fill="transparent"
          ...
          <div className="absolute flex flex-col items-center justify-center text-center px-4 w-full">
            <span className="text-[9.5px] font-black text-[#64748B] dark:text-slate-400 uppercase tracking-wider mb-0.5">
              {isFasting ? (isCompleted ? '단식 미션 격파!' : '단식 달성까지') : '단식 타이머 대기'}
            </span>
            <div className={`text-[25px] sm:text-[28px] font-extrabold font-mono tracking-tight transition-colors leading-none my-1 ${
              isFasting && !isCompleted ? 'text-[#1E293B] dark:text-white' : isCompleted ? 'text-emerald-500' : 'text-[#94A3B8] dark:text-[#64748B]'
            }`}>
              {formatTime(msLeft)}
            </div>
            <div className="text-[10px] font-extrabold text-[#64748B] dark:text-slate-400 mt-1 flex items-center gap-1 bg-neutral-50 dark:bg-slate-900 border border-neutral-150 dark:border-slate-700/60 px-2 py-0.5 rounded-full select-none leading-none">
              목표: <span className="font-black text-[#3B82F6]">{selectedPlan.fast}시간</span>
            </div>
          </div>
        </div>
        ...
        {!showStopConfirm ? (
          ...
          >
            {isFasting ? (
              ...
              <><Play className="w-5 h-5 fill-current" /> 지금 설정으로 단식 시작</>
            )}
          </button>
        ) : (
          ...
          <div className="mt-6 w-full bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-950 p-4.5 rounded-2xl flex flex-col gap-3.5 animate-in slide-in-from-top-3 duration-250">
            <div className="flex items-start gap-2 text-rose-800 dark:text-rose-200">
              ...
              <div className="min-w-0">
                <p className="text-xs font-extrabold">🚨 단식을 정말 중단하시겠습니까?</p>
                <p className="text-[10px] text-rose-600/80 dark:text-rose-300/80 mt-1 leading-snug">
                  중단하시면 지금까지의 단식 흐름이 중단되고 기록 창이 초기화됩니다.
                </p>
              </div>
            </div>
