...
import { Search, Plus, Sparkles, X, ChevronRight, ChevronLeft, Check, Loader2, Globe, Trash2, Minus } from 'lucide-react';
...
export default function MealLogger({ onAddLog, dateStr, onOpenCopyModal, onApplyRoutine, activeMealTime }: MealLoggerProps) {
  ...
  // Mouse Drag to Scroll Categories
  ...
  const [selectedNoodleSub, setSelectedNoodleSub] = useState<'all' | 'ramen' | 'guksu' | 'buckwheat_etc'>('all');
  ...
  const userCustomPresets = recentFoods.filter(f => f.id.startsWith('recent-') || f.category === 'custom');
  const allAvailablePresets = [...userCustomPresets, ...FOOD_PRESETS];

  const filteredPresets = (selectedCategory === 'recent' && !searchTerm)
    ? recentFoods
    : allAvailablePresets.filter(p => {
        const activeCategory = (selectedCategory === 'recent') ? 'all' : selectedCategory;
        ...
        if (activeCategory === 'carbs' && selectedSubCategory !== 'all') {
          ...
          if (selectedSubCategory === 'rice') {
            matchesSubCategory = p.category === 'carbs' && !nameLower.includes('파스타') && !nameLower.includes('면') && !nameLower.includes('식빵') && !nameLower.includes('베이글') && !nameLower.includes('빵') && !nameLower.includes('라면') && !nameLower.includes('짜파게티') && !nameLower.includes('사발면') && !nameLower.includes('국수') && !nameLower.includes('냉면');
          } else if (selectedSubCategory === 'pasta') {
            matchesSubCategory = nameLower.includes('파스타');
          } else if (selectedSubCategory === 'noodle') {
            const isNoodle = (p.category === 'carbs' || p.category === 'etc_meal') && (nameLower.includes('면') || nameLower.includes('라면') || nameLower.includes('짜파게티') || nameLower.includes('사발면') || nameLower.includes('국수') || nameLower.includes('냉면') || nameLower.includes('두부면') || nameLower.includes('불닭') || nameLower.includes('비빔면') || nameLower.includes('너구리') || nameLower.includes('소바') || nameLower.includes('우동') || nameLower.includes('곤약') || nameLower.includes('컵누들') || nameLower.includes('소컵')) && !nameLower.includes('파스타');
            
            if (!isNoodle) {
              matchesSubCategory = false;
            } else if (selectedNoodleSub !== 'all') {
              if (selectedNoodleSub === 'ramen') {
                matchesSubCategory = nameLower.includes('라면') || nameLower.includes('짜파게티') || nameLower.includes('사발면') || nameLower.includes('불닭') || nameLower.includes('비빔면') || nameLower.includes('너구리') || nameLower.includes('컵누들') || nameLower.includes('소컵');
              } else if (selectedNoodleSub === 'guksu') {
                matchesSubCategory = nameLower.includes('국수') || nameLower.includes('칼국수') || nameLower.includes('우동') || nameLower.includes('소면') || nameLower.includes('사리');
              } else if (selectedNoodleSub === 'buckwheat_etc') {
                matchesSubCategory = nameLower.includes('메밀') || nameLower.includes('냉면') || nameLower.includes('소바') || nameLower.includes('곤약') || nameLower.includes('미역국수');
              }
            }
          } else if (selectedSubCategory === 'bread') {
            matchesSubCategory = p.category === 'bread' || nameLower.includes('빵') || nameLower.includes('베이글') || nameLower.includes('식빵') || nameLower.includes('크루아상') || nameLower.includes('바게트');
          }
        } else if (activeCategory === 'meat' && selectedSubCategory !== 'all') {
  ...
  useEffect(() => {
    try {
      ...
      if (stored) {
        ...
        // Fallback: show the first few pre-defined popular presets
        const fallbacks = FOOD_PRESETS.slice(0, 8);
        setRecentFoods(fallbacks);
      }
    } catch (e) {
      console.error(e);
    }
  ...
  const performFatSecretSearch = async (query: string) => {
    ...
    try {
      ...
      if (err.message && err.message.includes('credentials are not set')) {
        ...
        setFatSecretError('네트워크 오류 (또는 서버 재시작 중)입니다. 다시 시도해주세요.');
      } else if (err.message && err.message.includes('400 Bad Request')) {
        setFatSecretError('FatSecret 인증 실패 (invalid_client). Client ID / Secret 값이 올바른지 확인해주세요.');
      } else {
  ...
  const handleDeleteRecentFood = (id: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      localStorage.setItem('recent_logged_foods', JSON.stringify(updatedList));
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleSaveMeal = () => {
    ...
    const resolvedIcon = activePreset?.icon || getEmojiForFoodName(name);
    ...
    // Save to recently added list in localStorage!
    try {
      const stored = localStorage.getItem('recent_logged_foods');
      let recentList: any[] = stored ? JSON.parse(stored) : [];
      // Remove duplicates by name
      recentList = recentList.filter((item: any) => item.name.toLowerCase() !== name.toLowerCase());
      recentList.unshift({
        id: `recent-${Date.now()}`,
        name,
        category,
      ...
      localStorage.setItem('recent_logged_foods', JSON.stringify(recentList));
      ...
      {!activePreset && !isCustomMode ? (
        <div className="flex flex-col gap-3">
          <div className="flex flex-col gap-2">
            <h3 className="text-sm font-bold text-[#1E293B] dark:text-white">식단 추가</h3>
              ...
              <div className="grid grid-cols-1 gap-1.5 max-h-[180px] overflow-y-auto">
                {routines.length === 0 ? (
                  <div className="text-center py-5 bg-white rounded-lg border border-dashed border-[#BBF7D0] text-[11px] text-neutral-500 font-medium leading-relaxed">
                    저장된 식단 루틴이 없습니다.<br />
                    하단 <span className="font-extrabold text-[#16A34A]">[루틴]</span> 탭에서 루틴을 먼저 추가해보세요!
                  </div>
                ) : (
                  routines.map(r => (
                    <div
                      ...
                      <div className="min-w-0 flex-1 pr-2">
                        <p className="text-[11px] font-extrabold text-neutral-800 flex items-center gap-1.5 truncate">
                          <span className="text-xs">
                            {r.mealTime === 'breakfast' ? '☀️' : 
                             r.mealTime === 'lunch' ? '🌤️' : 
                             r.mealTime === 'dinner' ? '🌙' : '🍿'}
                          </span>
                          <span className="truncate">{r.name}</span>
                          <span className="text-[9px] text-[#16A34A] bg-[#E8F5E9] px-1.5 py-0.2 rounded font-medium shrink-0">
                            {r.mealTime === 'breakfast' ? '아침' : 
                             r.mealTime === 'lunch' ? '점심' : 
                             r.mealTime === 'dinner' ? '저녁' : '간식'}
                          </span>
                        </p>
                        <p className="text-[9px] text-neutral-500 font-mono mt-0.5 truncate">
                          음식 {r.foods.length}개 · 총 {r.foods.reduce((sum, f) => sum + f.calories, 0)}kcal (탄:{Math.round(r.foods.reduce((sum, f) => sum + f.carbs, 0))}g 단:{Math.round(r.foods.reduce((sum, f) => sum + f.protein, 0))}g 지:{Math.round(r.foods.reduce((sum, f) => sum + f.fat, 0))}g)
                        </p>
                      </div>
                      <button
                        type="button"
          ...
          <div className="flex flex-col gap-2">
            <div className="flex gap-2 items-center relative" ref={searchContainerRef}>
              ...
              <div className="flex items-center gap-1.5 shrink-0 px-1">
                <span className={`text-[10px] font-bold ${isFatSecretMode ? 'text-blue-500' : 'text-neutral-400'}`}>FatSecret</span>
                <button 
                  type="button"
                  onClick={() => {
                ...
                >
                  <div className={`w-3.5 h-3.5 bg-white rounded-full absolute top-[3px] transition-all duration-200 ease-in-out ${isFatSecretMode ? 'left-[18px]' : 'left-[3px]'}`} />
                </button>
              </div>
            </div>
          </div>
            ...
            <div className="relative flex items-center -mx-4">
              ...
              >
                <button
                  ...
                  className={`flex-shrink-0 text-[11px] font-bold px-4 py-1.5 rounded-full transition-all cursor-pointer ${
            ...
            {selectedCategory === 'carbs' && (
              <div className="flex flex-col gap-2 mb-3.5 px-1">
                <div className="flex gap-1.5 overflow-x-auto pb-1 no-scrollbar justify-center">
                  {[
                    ...
                    <button
                      ...
                      className={`text-[10.5px] font-extrabold px-3.5 py-1.5 rounded-xl transition-all cursor-pointer border ${
                ...
                {selectedSubCategory === 'noodle' && (
                  <div className="flex gap-1 overflow-x-auto justify-center py-1 mt-1 animate-in slide-in-from-top-1 duration-200">
                    {[
                      { id: 'all', label: '🍜 면전체' },
                      ...
                      <button
                        key={subSub.id}
                        type="button"
                        onClick={() => setSelectedNoodleSub(subSub.id as any)}
                        className={`text-[10px] font-black px-2.5 py-1.5 rounded-lg transition-all border cursor-pointer ${
            ...
            {/* Food Presets Click Grid (Zero photorealism, high touch contrast) */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mt-1">
                  ...
                  >
                    <div className="min-w-0 flex-1 flex items-center gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-[10.5px] sm:text-[11px] font-black text-[#1E293B] tracking-tight leading-tight line-clamp-2 mb-0.5" title={preset.name}>
                          {preset.name}
                        </p>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[10.5px] font-extrabold text-[#3B82F6] leading-tight line-clamp-1 pr-1">커스텀 음식 추가</p>
        ...
        <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom duration-250">
            ...
            <div className="flex items-center gap-1.5 w-full justify-between">
              <div className="flex items-center gap-1.5 min-w-0">
                <span className="text-base flex-shrink-0">
                  {activePreset ? activePreset.icon : getEmojiForFoodName(customName)}
                </span>
                <span className="text-xs font-black text-[#1E293B] truncate">
                  {activePreset ? `${activePreset.name} 추가하기` : '커스텀 음식 추가하기'}
                </span>
              </div>
          ...
          <div className="flex flex-col gap-3">
            {!activePreset && (
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <div className="w-full sm:w-44">
                  <label className="text-[10px] font-extrabold text-[#64748B] block mb-1">식품 분류 (카테고리)</label>
                  <select
            ...
            {activePreset && (activePreset.id.startsWith('recent-') || activePreset.category === 'custom') && (
              <button
                type="button"
                onClick={(e) => {
